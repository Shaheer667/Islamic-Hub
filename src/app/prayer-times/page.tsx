import PrayerTimesClient from "@/components/PrayerTimesClient";

export default function PrayerTimesPage() {
  return (
    <main className="min-h-screen bg-[#071812]">
      <section className="mx-auto max-w-7xl px-5 py-14">
        <div className="mb-10">
          <p className="mb-3 text-sm font-semibold uppercase tracking-[0.3em] text-amber-400">
            Daily Namaz Timings
          </p>

          <h1 className="text-4xl font-bold text-white">Prayer Times</h1>

          <p className="mt-4 max-w-2xl text-white/60">
            Check daily prayer times by city and country. You can select
            calculation method according to your region.
          </p>
        </div>

        <PrayerTimesClient />
      </section>
    </main>
  );
}